...
author: "Lab Created By Karen Nylund-Gibson, Example from Nylund-Gibson et al. (2022)"
date: "2024-11-18"
...
Example is related (but not exactly the same) from *The Utility of Latent Class Analysis to Understand Heterogeneity in Youth Coping Strategies: A Methodological Introduction* (Nylund-Gibson et al., 2022) published in Behavior Disorders. See here: https://doi.org/10.1177/01987429211067214
...
output_bd <- readModels(here("enum_total"), quiet = TRUE)
...
### 3-Class Probability Plot (Entire Sample)
...
plot_lca(model_name = output_bd$c3_bd.out)
```
...
male_plot <- plot_lca(model_name = output_males$c3_bd_males.out)
male_plot
```
